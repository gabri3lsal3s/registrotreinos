...
| **`MuscleGroupIcon`** | `{ muscleGroup?, exerciseName?, className?, size?, withContainer?, containerClassName? }` | Renderiza o **ícone semântico do Lucide** correspondente ao grupo muscular (ou inferido pelo nome do exercício via `getMuscleGroupMeta`), com suporte a container temático colorido. |
| **`MuscleGroupBadge`** | `{ muscleGroup?, exerciseName?, className?, size?, showLabel? }` | Badge estilizada (ícone + label) com classes de alto contraste da paleta temática de cada grupo muscular. |
